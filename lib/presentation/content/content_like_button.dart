...
class _ContentLikeButtonState extends State<ContentLikeButton> {
  ...
  void didUpdateWidget(ContentLikeButton oldWidget) {
    ...
    if (widget.selected != oldWidget.selected) {
      if (widget.selected) {
        likeContentWithCheckLogin(widget.postId, widget.contentId).then((successToLike) => {
          if (successToLike) {
            statesController.update(MaterialState.selected, widget.selected)
          }
        });
      } else {
        cancelLikeContentWithCheckLogin(widget.postId, widget.contentId).then((successToCancel) => {
          if (successToCancel) {
            statesController.update(MaterialState.selected, widget.selected)
          }
        });
